...
func TestSimpleColumnsTableInfoDecoder(t *testing.T) {
	...
	decoder := &columnsTableInfoDecoder{}
	require.True(t, decoder.tryDecodeSimpleColumnsTableInfo(data, actual))
	...
	require.NoError(t, err)
	require.False(t, decoder.tryDecodeSimpleColumnsTableInfo(data, actual))
	...
	expected = &model.TableInfo{}
	require.NoError(t, decodeColumnsTableInfo(data, expected))
	require.NoError(t, decoder.decode(data, actual))
	require.Equal(t, expected, actual)
	require.NotNil(t, actual.View)

	require.Positive(t, decoder.retainedMemory())
	decoder.close()
	require.Zero(t, decoder.retainedMemory())
}
